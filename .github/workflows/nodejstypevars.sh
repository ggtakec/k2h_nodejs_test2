...
if [ -z "${CI_OSTYPE}" ]; then
	...
	if /bin/sh -c "${SUDO_CMD} ${INSTALLER_BIN} ${INSTALL_CMD} ${INSTALL_AUTO_ARG} dnf-plugins-core"; then
		if /bin/sh -c "${SUDO_CMD} ${INSTALLER_BIN} config-manager --set-enabled powertools"; then
			...
			# Add gcc packages
			#
			if /bin/sh -c "${SUDO_CMD} ${INSTALLER_BIN} ${INSTALL_CMD} ${INSTALL_AUTO_ARG} gcc-toolset-11-gcc gcc-toolset-11-gcc-c++ scl-utils"; then
				#
				# Set environments
				#
				...
				echo "[ERROR] Failed to install \"gcc-toolset-11-gcc gcc-toolset-11-gcc-c++ scl-utils\". The script doesn't break here, but fails to node addon build."
			fi
		else
