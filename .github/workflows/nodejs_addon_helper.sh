...
remove_binaries_from_pacakage_json()
{
	...
	_KW_PREBUILDS_1='"prebuilds"'
	_KW_PREBUILDS_2='"prebuilds/"'
	_KW_NODE_BIN_1='"build/Release/\*.node"'
	_IN_FILES_SECTION=0
	while IFS= read -r _ONE_LINE || [ -n "${_ONE_LINE}" ]; do
		if [ "${_IN_FILES_SECTION}" -eq 0 ]; then
			...
			if printf '%s' "${_ONE_LINE}" | grep -q '^[[:space:]]*\][[:space:]]*'; then
				_IN_FILES_SECTION=0
				printf '%s\n' "${_ONE_LINE}"
			elif printf '%s' "${_ONE_LINE}" | grep -q -e "[[:space:]]*${_KW_PREBUILDS_1}[[:space:]]*" -e "[[:space:]]*${_KW_PREBUILDS_2}[[:space:]]*" -e "[[:space:]]*${_KW_NODE_BIN_1}[[:space:]]*"; then
